<x-structure.wrapper title="Home">
    ...
    @if(session()->get('PASSWORD_CHANGED', null))
        <x-message.success title="Password Changed" description="{{ session()->get('PASSWORD_CHANGED') }}" />
    @endif
    {{-- Account deleted message --}}
    @if(session()->get('ACCOUNT_DELETED', null))
        <x-message.success title="Account Deleted" description="{{ session()->get('ACCOUNT_DELETED') }}" />
    @endif
    {{-- Account leave message --}}
    @if(session()->get('LEAVE_SUCCESS', null))
        <x-message.success title="Left Course" description="{{ session()->get('LEAVE_SUCCESS') }}" />
    @endif
