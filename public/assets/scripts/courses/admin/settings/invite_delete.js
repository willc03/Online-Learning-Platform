$("#invite-manager #invite-delete").on("click", function () {
    const button = $(this);
    ...
    if ( $(button).data("shouldDelete") === true ) {
        $.ajax({
            headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') }, method: "DELETE", url: inviteRemoveRoute, data: {
                inviteId: $(row).attr('id')
            }, success: function () {
                // Hide the row
                $(row).css({
