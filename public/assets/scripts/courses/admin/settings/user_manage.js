$("#user-manager #user-delete").on("click", function () {
    ...
    if ( $(button).data("shouldDelete") === true ) {
        $.ajax({
            headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') }, method: "DELETE", url: userRemoveRoute, data: {
                userId: $(row).attr('id')
            }, success: function () {
                // Hide the row
                $(row).css({
...
$(blockBtns.on({
    mouseenter: function () {
        ...
        // Click behaviour
        $.ajax({
            headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') }, method: "POST", url: userBlockRoute, data: {
                userId: userId, modificationType: 'activeState'
