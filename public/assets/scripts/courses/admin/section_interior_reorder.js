$("#reorder-section-button").on("click", function () {
    ...
    const sectionSorter = $(section).sortable({
        revert: 0, placeholder: "course_section_placeholder", opacity: 0.5, cancel: false, axis: 'y', tolerance: 'pointer', appendTo: $(section)
    });
    ...
    if ( $(this).attr("data-active") === "false" ) {
        ...
        $.ajax({
            headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') }, method: "POST", url: ajaxRoute, data: {
                'course_id': courseId, 'edit_type': 'section_interior_order', 'section_id': section_id, 'data': orderJson
            }
        });
    }
});
